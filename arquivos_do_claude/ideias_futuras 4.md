...
**Achados parciais (14/07/2026), com o proxy incompleto disponível hoje:**
- Descanso entre jogos (só liga, sem copas): correlação ~0 com saldo de
  gols — mas inconclusivo, o ruído de não saber sobre jogos de copa pode
  esconder o efeito real. Não descartar, só documentar limitação.
- Fadiga de fim de temporada: **contraintuitivo** — gols totais SOBEM no
  fim de temporada (2.92 nos últimos 20% dos jogos vs 2.73-2.85 no resto
  da temporada), não descem. Interpretação provável: não é fadiga física,
  é **efeito de contexto/importância da partida** (times já rebaixados ou
  já campeões jogando sem pressão, abrindo o jogo; brigas de
  rebaixamento/título gerando jogos mais decididos). Sugere que
  "importância do jogo" (distância pra zona de rebaixamento/título) pode
  ser um sinal mais promissor que fadiga física pura — e esse já é
  calculável com o que temos hoje (posição na tabela), sem precisar da
