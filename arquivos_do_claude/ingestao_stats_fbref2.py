...
_TOKENS_IGNORADOS = {
    ...
    "ud", "rcd", "ec", "ca", "cr", "se", "sp", "club", "clube", "real",
    "athletic", "1846", "1904", "04", "05", "96",
}
...
def normalizar(nome: str) -> str:
    ...
    nome = nome.lower()
    tokens = [t for t in nome.split() if t not in _TOKENS_IGNORADOS]
    return " ".join(tokens)
...
def match_times(norm_fb: str, norm_para_id: dict) -> tuple[str | None, str]:
    ...
    for candidato in norm_para_id:
        ...
        if tokens_fb and (tokens_fb <= tokens_c or tokens_c <= tokens_fb):
...
def main():
    ...
    base = prepara(schedule, {"xg": ["xg"], "xga": ["xga"], "possession": ["poss"]})
