#!/usr/bin/env python3
"""Walk-forward cross-validation v11 -- clone de walkforward_cv_v9.py,
mesma estrutura de folds/stacking/calibração, só troca o feature set
(FEATURES_V9 -> FEATURES_V11) e os nomes de modelo/tabela.

FEATURES_V11 = FEATURES_V10 (tudo de v9 + idade/altura/venue) + força do XI
titular com dois sinais temporais paralelos, "abertura" (XI previsto por
backtest_xi_walkforward.py, sem vazamento, disponível ANTES da escalação
real sair) e "fechamento" (XI real confirmado, match_lineup_fotmob) -- ver
comentário completo em dados_historicos.py. Diferente de v9->v10 (que
nunca teve validação walk-forward própria, só entrou "cru" no loop diário
de rodar_predicoes.py), v11 nasce com este script justamente pra medir
com rigor se a força do XI titular (real/previsto) ajuda o modelo
principal de resultado de partida -- cobertura esparsa de XI já fez o
projeto excluir isso da cadeia antes (ver comentário da v6 em
dados_historicos.py), então não dá pra assumir que ajuda sem medir.
...
Por fold e por modelo (catboost_v11 / xgboost_v11 / lightgbm_v11 / mlp_v11):
  - Treina no Train set com FEATURES_V11
...
Stacking v11 (duas variantes para 1X2; apenas stacking_v11 para mercados binários):
  - stacking_v11:         4 base models → meta-features (n_val_total, 4×3=12 / 4×2=8)
  - stacking_v11_sem_mlp: 3 base models (sem MLP) → (n_val_total, 3×3=9) — apenas 1X2
...
logger = logging.getLogger("walkforward_cv_v11")
...
MODELOS_BASE = ["catboost_v11", "xgboost_v11", "lightgbm_v11", "mlp_v11"]
MODELOS_SEM_MLP = ["catboost_v11", "xgboost_v11", "lightgbm_v11"]
...
def treinar_mercado_binario(
    ...
    if all(len(oof_probs_por_modelo[m]) == len(FOLDS) for m in MODELOS_BASE):
        ...
        if len(test_probs_por_modelo) == len(MODELOS_BASE):
            ...
            logger.info("  JSON:resultado_binario:%s", json.dumps({
                "market": market_str, "modelo": "stacking_v11", "fold": "fold_3",
            ...
            predicoes.extend(construir_predicoes_mercado(
                test_df_f3, stacking_probs, stacking_classes, "stacking_v11", market_str, mapa_selecao
            ))
    else:
...
def main() -> None:
    ...
    logger.info("=== Walk-forward CV v11 iniciado ===")
    ...
    for fold in FOLDS:
        ...
        for nome in MODELOS_BASE:
            ...
            predicoes_v11.extend(construir_predicoes(test_df, test_probs, test_classes, nome))
            ...
            if fold_nome == "fold_3" and nome == "catboost_v11":
    ...
    if all(len(oof_probs_por_modelo[m]) == len(FOLDS) for m in MODELOS_BASE):
        ...
        if len(test_probs_por_modelo) == len(MODELOS_BASE):
            ...
            predicoes_v11.extend(construir_predicoes(test_df_fold3, stacking_probs, stacking_classes, "stacking_v11"))

        # --- Variante sem MLP (CatBoost + XGBoost + LightGBM) ---
        if all(m in test_probs_por_modelo for m in MODELOS_SEM_MLP):
            logger.info("Treinando stacking_v11_sem_mlp (sem MLP)...")
            ...
            predicoes_v11.extend(construir_predicoes(test_df_fold3, probs_sem_mlp, classes_sem_mlp, "stacking_v11_sem_mlp"))
    else:
    ...
    if modelo_catboost_fold3 is not None:
        ...
        features_catboost = ml.FEATURES_POR_MODELO["catboost_v11"]
    ...
    limpar_tabela(supabase_client, "model_v11_walkforward_results")
    if resultados_wf:
        inserir_em_lotes(supabase_client, "model_v11_walkforward_results", resultados_wf)
        logger.info("  model_v11_walkforward_results: %d linhas", len(resultados_wf))

    limpar_tabela(supabase_client, "model_v11_feature_importance")
    if importancia_linhas:
        inserir_em_lotes(supabase_client, "model_v11_feature_importance", importancia_linhas)
        logger.info("  model_v11_feature_importance: %d linhas", len(importancia_linhas))

    limpar_tabela(supabase_client, "model_v11_feature_coverage")
    if cobertura:
        inserir_em_lotes(supabase_client, "model_v11_feature_coverage", cobertura)
        logger.info("  model_v11_feature_coverage: %d linhas", len(cobertura))

    # model_predictions — previsões OOF por partida (SimulacaoCarteira / backtest)
    MODELOS_V11 = ["catboost_v11", "xgboost_v11", "lightgbm_v11", "mlp_v11", "stacking_v11", "stacking_v11_sem_mlp"]
    MODELOS_V11_CAL = [f"{m}_calibrado_{mt}" for m in MODELOS_BASE for mt in METODOS_CALIBRACAO]
    logger.info("Limpando previsões v11 anteriores de model_predictions...")
    for _mv11 in MODELOS_V11 + MODELOS_V11_CAL:
        _executar_com_retry(
            lambda mv=_mv11: supabase_client.table("model_predictions")
                .delete().eq("model_name", mv).eq("market", "1x2").execute()
        )
    if predicoes_v11:
        inserir_em_lotes(supabase_client, "model_predictions", predicoes_v11)
        logger.info("  model_predictions (v11 OOF 1X2): %d linhas", len(predicoes_v11))

    # Previsões dos mercados binários (OU25 e BTTS)
    for cfg in MERCADOS_BINARIOS:
        mkt = cfg["market"]
        logger.info("Limpando previsões v11 %s de model_predictions...", mkt)
        for _mv11 in MODELOS_V11 + MODELOS_V11_CAL:
            _executar_com_retry(
                lambda mv=_mv11, m=mkt: supabase_client.table("model_predictions")
    ...
    if predicoes_binarios:
        ...
        logger.info("  model_predictions (v11 OOF binários): %d linhas", len(predicoes_binarios))
    ...
    if calibracao_linhas:
        logger.info("Atualizando model_calibration para modelos v11 (%d linhas)...", len(calibracao_linhas))
        for mv11 in MODELOS_BASE:
            _executar_com_retry(
                lambda mv=mv11: supabase_client.table("model_calibration")
                    .delete().eq("model_name", mv).execute()
            )
        inserir_em_lotes(supabase_client, "model_calibration", calibracao_linhas)
        logger.info("  model_calibration (v11): %d linhas", len(calibracao_linhas))

    logger.info("=== Walk-forward CV v11 concluído ===")
