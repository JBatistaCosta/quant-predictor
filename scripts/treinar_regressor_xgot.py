...
espírito de `treinar_regressor_xg.py` (mesmo dataset, mesmo split
cronológico 80/20, só grava previsão do holdout, treina v1+v2 por
execução) -- só troca o alvo pra `xgot_home`/`xgot_away`.
...
v2 usa `FEATURES_XG_XI_V2` (v1 + força do XI titular na data da partida --
ver docstring de `treinar_regressor_xg.py` pro detalhe completo).
...
MODEL_NAMES = ["catboost_xgot_regressor_v1", "catboost_xgot_regressor_v2"]
FRACAO_TESTE = 0.2
    ...
    for model_name in MODEL_NAMES:
        ...
        for lado, coluna_alvo in (("home", "xgot_home"), ("away", "xgot_away")):
            ...
            if treino.empty or teste.empty:
                print(f"[{model_name}/{lado}] sem dados suficientes de xGOT real pra treinar/testar -- pulando.")
                continue
