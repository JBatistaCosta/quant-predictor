...
- `teams`/`leagues`/`matches`/`match_stats`/`odds_market`/`model_predictions`/`team_strengths`/`league_model_params`/`team_elo`/`team_elo_history`/`players` e as tabelas `*_fotmob` — pipeline de dados reais ingerido automaticamente (RLS leitura pública, escrita só via `SUPABASE_SERVICE_ROLE_KEY`, nunca pela chave anon do frontend).
...
## Guidance genérica de AWS (PR #219, mesclado depois — revisar aplicabilidade)

Conteúdo abaixo veio de um PR de 2026-08-09 que importou boilerplate do
"AWS Agent Toolkit" (`setup-instructions/setup.md` do repo
`aws/agent-toolkit-for-aws`) — não foi validado contra o setup real deste
projeto (que usa Vercel como deploy principal; a doc de AWS Elastic
Beanstalk em `README.md` é um experimento à parte). Referências como
"AWS MCP Server", `retrieve_skill`, skill `aws-secrets-manager` e
`asm-exec` podem não corresponder a nenhuma ferramenta disponível nesta
sessão — trate como referência, não como instrução obrigatória, até
confirmar que essas ferramentas existem no ambiente atual.
