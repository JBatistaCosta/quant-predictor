...
- `model_betting_strategy` é a config versionada (lookup, não modelo de ML) de qual estratégia seguir por mercado/linha em cartões e escanteios — `seguir_modelo`/`anti_modelo`/`nenhuma`/`nunca`, com `confianca`, se foi `validado_carteira` (simulação de carteira cronológica, não só log-loss/ROI agregado) e `evidencia_anti_perde` (IC95% confirma que apostar CONTRA o modelo perde, mesmo sem edge positivo em nenhum lado). Reavaliar conforme o cron acumula mais partidas de 2026 — ver `CONTEXTO_PROJETO.md` para o histórico completo por trás de cada linha.
