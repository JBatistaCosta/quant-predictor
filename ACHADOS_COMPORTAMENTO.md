...
**Drift de schema.** `match_lineup_fotmob.formation` e `.team_rating` existem em produção mas nenhum arquivo em `supabase/migrations/` as cria. O `comment on column` da fase 1 precisou ficar dentro de um guard de `information_schema` para não quebrar num replay limpo. Vale uma varredura de `information_schema` contra as migrations atrás de outros casos.
